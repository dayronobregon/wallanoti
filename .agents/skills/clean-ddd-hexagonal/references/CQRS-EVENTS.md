...
### Mandatory Flow (for this skill)

All commands and queries are dispatched through a **Mediator** in the application layer.

```
Controller/API/Consumer -> Mediator -> Command|Query Handler -> Domain/Interfaces
```

Rules:
- No direct controller -> handler invocation
- One handler per command/query
- Handlers stay thin: orchestrate domain + interfaces, avoid domain logic leakage
...
class PlaceOrderHandler:
    ...
    handle(command: PlaceOrderCommand) -> OrderId:
        ...
        db.transaction((tx) => {
          ...
          for event in order.PullDomainEvents():
...
Start simple—same database, mediator-based dispatch:
...
// apps/Api layer
await mediator.send<OrderId>(placeOrderCommand);
await mediator.send<OrderDTO | null>(getOrderQuery);
```

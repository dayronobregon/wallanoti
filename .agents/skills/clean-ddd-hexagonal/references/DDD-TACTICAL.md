...
abstract class AggregateRoot<ID> extends Entity<ID>:
    ...
    Record(event: DomainEvent):
        this._domainEvents.append(event)

    PullDomainEvents() -> List<DomainEvent>:
        events = this._domainEvents
        this._domainEvents = []
        return events
...
class Order extends AggregateRoot<OrderId>:
    ...
    static create(customerId: CustomerId) -> Order:
        ...
        order.Record(OrderCreated{orderId, customerId})
        return order
    ...
    addItem(productId, quantity, unitPrice):
        ...
        this.Record(OrderItemAdded{orderId, productId, quantity})
    ...
    removeItem(productId):
        ...
        this.Record(OrderItemRemoved{orderId, productId})
    ...
    confirm():
        ...
        this.Record(OrderConfirmed{orderId, total})

    ship(trackingNumber):
        guard: status == CONFIRMED

        this.status = SHIPPED
        this.Record(OrderShipped{orderId, trackingNumber})

    cancel(reason: string):
        guard: status not in [SHIPPED, DELIVERED]

        this.status = CANCELLED
        this.Record(OrderCancelled{orderId, reason})
...
Publishing collected aggregate events:
...
Event recording in aggregate methods:

```
this.Record(NewItemsFoundEvent{
    aggregateId,
    occurredAt,
    userId,
    items
